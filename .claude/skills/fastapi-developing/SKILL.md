...
description: FastAPI 0.115+ async API developing with Pydantic v2 validation (model_validate, model_dump, ConfigDict), dependency injection (Annotated[T, Depends]), lifespan context managers, and async database patterns. Use when creating routes, endpoints, schemas, BaseModel classes, routers, middleware, or fixing HTTPException errors. Apply for .py files in api/, routes/, services/, main.py, or files with "from fastapi import". (project)
---
